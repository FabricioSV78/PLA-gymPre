...
function Reveal({
  ...
  const reduceMotion = !SITE_MOTION_ENABLED;
...
export function GymExperience() {
  ...
  useEffect(() => {
    document.body.style.overflow = menuOpen || coachModalId ? "hidden" : "";
    return () => { document.body.style.overflow = ""; };
  }, [menuOpen, coachModalId]);
  ...
  const handleTrial = (event: FormEvent<HTMLFormElement>) => {
    ...
    window.open(
      whatsappUrl(`Hola, soy ${name}. Quiero activar mi semana de experiencia en Gym Rat Club.\nCelular: ${phone}\nObjetivo: ${goal}\nFecha ideal: ${date}`),
      "_blank",
      "noopener,noreferrer",
    );
  };
  ...
  return (
    <div ref={rootRef} className="site-shell" onPointerMove={handlePointerMove}>
      ...
      <main id="contenido">
        <section className="hero" id="inicio">
          <video className="hero__video" autoPlay muted loop playsInline preload="metadata" poster="/media/club.webp" aria-label="Entrenamiento de fuerza en Gym Rat Club">
            ...
            <Reveal className="schedule" direction="scale">
              ...
              <AnimatePresence mode="wait">
                <motion.div key={selectedDay} className="schedule__sessions" initial={prefersReducedMotion ? false : { opacity: 0, y: 16, scale: 0.985 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={prefersReducedMotion ? undefined : { opacity: 0, y: -10, scale: 0.99 }} transition={{ duration: prefersReducedMotion ? 0 : 0.28, ease: [0.22, 1, 0.36, 1] }}>
                  {schedule[selectedDay].map((session) => { const [time, ...name] = session.split(" "); return <div key={session}><Clock3 aria-hidden="true" /><strong>{time}</strong><span>{name.join(" ")}</span><button type="button" onClick={() => window.open(whatsappUrl(`Hola, quiero reservar ${name.join(" ")} el ${selectedDay} a las ${time}.`), "_blank", "noopener,noreferrer")} aria-label={`Reservar ${name.join(" ")} a las ${time}`}><Plus /></button></div>; })}
                </motion.div>
              </AnimatePresence>
            </Reveal>
          </div>
            ...
            <div className="facilities-grid">
              {facilities.map((facility, index) => (
                <Reveal key={facility.title} className={facility.className} delay={index * 0.05} direction={index % 3 === 0 ? "left" : index % 3 === 1 ? "right" : "scale"}>
                  <Image src={facility.src} alt={facility.alt} fill sizes="(max-width: 700px) 92vw, 50vw" />
              ...
              {plans.map((plan, index) => {
                ...
                return (
                  <Reveal key={plan.name} className={`plan-card ${plan.featured ? "is-featured" : ""}`} delay={index * 0.065} direction={index === 0 ? "left" : index === 1 ? "scale" : "right"}>
                    ...
                    <div className="plan-card__price"><small>S/</small><motion.strong key={price} initial={{ opacity: 0, y: 12, scale: 0.94 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }}>{price}</motion.strong><span>/ mes</span></div>
        ...
        <section className="section faq">
          <div className="container faq__grid">
            ...
            <Reveal className="faq__list" direction="right">
              {faqs.map(([question, answer], index) => <details key={question} open={index === 0}><summary>{question}<span className="faq__plus"><Plus className="plus" /><Minus className="minus" /></span></summary><p>{answer}</p></details>)}
            </Reveal>
          </div>
        </section>
        ...
        {modalCoach ? (
          ...
          >
            <motion.div
              ...
              initial={{ opacity: 0, y: 42, scale: 0.94, rotate: -1.5 }}
            ...
            >
              ...
              <div className="coach-modal__visual">
                <Image src={modalCoach.image} alt={`${modalCoach.name}, ${modalCoach.role}`} fill sizes="(max-width: 700px) 94vw, 46vw" style={{ objectPosition: modalCoach.imagePosition }} />
      ...
      <a className={`mobile-cta ${showMobileCta ? "is-visible" : ""}`} href="#experiencia" aria-hidden={!showMobileCta} tabIndex={showMobileCta ? undefined : -1}><span><strong>Empieza gratis</strong></span><ArrowRight /></a>
    </div>
  );
}
